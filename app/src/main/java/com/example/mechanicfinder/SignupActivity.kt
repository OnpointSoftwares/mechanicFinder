...
class SignupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        auth.currentUser.also {
            if (it != null) {
                user = it
                startActivity(Intent(this,CarOwnerMapActivity::class.java))
            }
        }
            ...
            auth.createUserWithEmailAndPassword(
                ...
                .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        ...
                        ref.child("carOwner").child(user.uid).setValue(User(etNameSignup.editText?.text.toString(),etEmailIdSignup.editText?.text.toString(),etPasswordSignup.editText?.text.toString(),0.011.toString(),0.22.toString())).addOnCompleteListener {
                            if(it.isSuccessful)
                            {
